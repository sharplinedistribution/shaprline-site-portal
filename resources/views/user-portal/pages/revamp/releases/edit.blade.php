         ...
         <div class="card">
            ...
            <div class="card-body py-4 px-lg-4 px-2">
               <div class="mb-lg-4 mb-3 input-icon position-relative">
                  ...
                  {!! Form::select('number_of_songs', numberOfSongs(), count(collect(json_decode($release->track_ids))), ['class' => 'form-control custom-control ', 'id' => 'number_of_songs', 'required', 'disabled']) !!}
                </select>
         ...
         <div class="card mt-3">
            <div class="py-4 px-lg-4 px-2 sortable-container" id="uploadedTracks">
                ...
                @forelse ($tracks as $index=>$item)
                    <div class="sortable-item stories mt-2 border p-3" id="remove_track_{{ $item->id }}">
                        <div class="row">
                            <div class="col-1">
                                <small>&nbsp;</small>
                                <h4 >{{ ++$index }}</h4>
                            ...
                            <div class="col-9">
                                ...
                                <h4 id="track_name_{{ $item->id }}" style="color: var(--primary)">{{ $item->title }} @if(isset($item->track_version))({{ $item->track_version }})@endif</h4>
                            </div>
                            <div class="col-md-2">
                                <div class="row">
                        ...
                        <input type="hidden" value="{{ $item->id }}" id="track_sortable_{{ $item->id }}" name="audio_track_sortable[]" >
                    </div>
                @empty
                @endforelse
            </div>
...
<script>
   ...
   function multiTrack(count, trackId, track_title) {
    //   $("#multiTracks").append('<div class="card-body py-4 px-lg-4 px-2"><div class="row mt-5  " id="track_msg_' + count + '" > <div class="col-md-12"> <h2 class="text-center trackUploaded" ><i class="fa fa-check text-success"></i>&nbsp;&nbsp;('+count+') ' + track_title + ' Uploaded</h2> <div class="text-center"> <a href="javascript:;" onClick="editTrack(' + trackId + ',$(this))"  data-count="' + count + '" data-name="' + track_title + '"><i class="fa fa-pencil text-success d-none" style="font-size:25px"></i></a>&nbsp;&nbsp;<a href="javascript:;" onClick="deleteTrack(' + trackId + ',$(this))" data-count="' + count + '" data-name="' + track_title + '"><i class="fa fa-trash text-danger" style="font-size:25px;"></i></a> </div></div> </div></div>');
      $("#uploadedTracks").append('<div id="remove_track_'+trackId+'" class="stories mt-2 border p-3"><div class="row"><div class="col-1"><small>&nbsp;</small><h4 >'+count+'</h4></div><div class="col-9"><small>Track Name</small><h4 id="track_name_'+trackId+'"style="color: var(--primary)">'+track_title+'</h4></div><div class="col-md-2"><div class="row"><div class="col-6 text-center"><small>Edit</small><br><a href="#multiTracks" onClick="editTrackv2('+trackId+','+count+')"><span><i class="fa fa-pencil-square" style="color:var(--primary); font-size:30px"></i></span></a></div><div class="col-6"><small>Remove</small><br><a href="javascript:;" onClick="removeTrack('+trackId+',$(this),{{ $release->id }})"><span><i class="fa fa-minus-square" style="color:var(--primary); font-size:30px;"></i></span></a></div></div></div></div></div>');
   }
   ...
   function removeTrack(id, elm, release_id) {
      ...
      var audio_track_old = $("[name='audio_track[]']").map(function() {
         return $(this).val();
      }).get();
      ...
      if (conf == true) {
        if(audio_track.length >= 2){
            ...
            notify('To delete the tracks, there must be more than one track.', 'error');
        }
